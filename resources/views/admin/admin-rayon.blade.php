...
<div class="mt-4">
	<h1 style="margin-botton: 20px;">Admin Rayon</h1>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="addForm" name="addForm" class="form-horizontal">
					<input type="hidden" name="admin_rayon_id" id="admin_rayon_id">
					<div class="form-group">
						<label for="nama" class="col-sm-12 control-label">Nama Admin Rayon</label>
						<div class="col-sm-12">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="deleteForm" name="deleteForm" class="form-horizontal">
					<input type="hidden" name="admin_rayon_id_delete" id="kom-id-delete">
					<div class="container">
...
<script type="text/javascript">
	$(function () {
		...
		$('#createBtn').click(function () {
			$('#saveBtn').val("add admin-rayon");
			$('#admin_rayon_id').val('');
			$('#addForm').trigger("reset");
			$('#addModalHeader').html("Tambah Admin Rayon");
			$('#addModal').modal('show');
		});

		$('body').on('click', '.edits', function () {
			var admin_rayon_id = $(this).data('id');
			$.get("{{ route('admin-rayon.index') }}" +'/' + admin_rayon_id +'/edit', function (data) {
				$('#addModalHeader').html("Edit admin-Rayon");
				$('#saveBtn').val("edit admin-Rayon");
				$('#addModal').modal('show');
			})
		});
		...
		$('#saveBtn').click(function (e) {
			e.preventDefault();
			$(this).html('Memproses..');
			admin_rayon_id = $('#id_admin_kom').val();
			$.ajax({
				data: $('#addForm').serialize(),
				url: "/admin/admin-rayon/tambah/"+admin_rayon_id,
				type: "POST",
				dataType: 'json',
		...
		$('body').on('click', '.deletes', function () {
			...
			if(confirm("Are You sure want to delete !")){
				$.ajax({
					type: "POST",
					url: "/admin/admin-rayon/hapus/"+admin_rayon_id,
					success: function (data) {
						table.draw();
					},
					error: function (data) {
		...
		// 	var admin_rayon_id = $(this).data('id');
		// 	$.get("{{ route('admin-rayon.index') }}" +'/' + admin_rayon_id +'/edit', function (data) {
		// 		$('#deleteModalHeader').html("Hapus admin-Rayon");
		// 		$('#deleteBtn').val("edit kom");
		// 		$('#deleteModal').modal('show');
		// 		$('#kom-id-delete').val(data.id_admin-Rayon);
		...
		// 	$.ajax({
		// 		type: "DELETE",
		// 		url: "{{ route('admin-rayon.store') }}"+'/'+data,
		// 		dataType: 'json',
		// 		success: function (data) {
		// 			table.draw();
		// 		},
